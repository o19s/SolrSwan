 ...
 *
 */
public class SpanAwareFastVectorHighlighter {
   ...
   */
  public SpanAwareFastVectorHighlighter(){
    this( DEFAULT_PHRASE_HIGHLIGHT, DEFAULT_FIELD_MATCH, new TreeSet<Integer>());
  }
   ...
   */
  public SpanAwareFastVectorHighlighter( boolean phraseHighlight, boolean fieldMatch, Set<Integer> docIds ){
    this( phraseHighlight, fieldMatch, new SimpleFragListBuilder(), new ScoreOrderFragmentsBuilder(), docIds );
  }
   ...
   */
  public SpanAwareFastVectorHighlighter( boolean phraseHighlight, boolean fieldMatch,
      FragListBuilder fragListBuilder, FragmentsBuilder fragmentsBuilder, Set<Integer> docIds ){
  ...
  /**
   * create a {@link SpanAwareFieldQuery} object.
   * 
   * @param query a query
   * @return the created {@link SpanAwareFieldQuery} object
   */
  public SpanAwareFieldQuery getFieldQuery( Query query, IndexReader reader, TreeSet<Integer> filterDocIds ) throws IOException {
    return new SpanAwareFieldQuery( query, reader, phraseHighlight, fieldMatch, filterDocIds );
  }

  /**
   * return the best fragment.
   * 
   * @param fieldQuery {@link SpanAwareFieldQuery} object
   ...
   */
  public final String getBestFragment( final SpanAwareFieldQuery fieldQuery, IndexReader reader, int docId,
 ...
 * @param encoder
 * @param fragmentsBuilder2 
 * @param fragListBuilder2 
   * @return created fragments or null when no fragments created.
   *         size of the array can be less than maxNumFragments
   * @throws IOException If there is a low-level I/O error
   */
  public final String[] getBestFragments( final SpanAwareFieldQuery fieldQuery, IndexReader reader, int docId,
      String fieldName, int fragCharSize, int maxNumFragments,  FragListBuilder fragListBuilder2, org.apache.lucene.search.vectorhighlight.FragmentsBuilder fragmentsBuilder2,
      String[] preTags, String[] postTags, Encoder encoder ) throws IOException {
    FieldFragList fieldFragList =
      getFieldFragList( fragListBuilder2, fieldQuery, reader, docId, fieldName, fragCharSize );
  ...
  private FieldFragList getFieldFragList( FragListBuilder fragListBuilder,
      final SpanAwareFieldQuery fieldQuery, IndexReader reader, int docId,
      String fieldName, int fragCharSize ) throws IOException {
    SpanAwareFieldTermStack fieldTermStack = new SpanAwareFieldTermStack( reader, docId, fieldName, fieldQuery );
